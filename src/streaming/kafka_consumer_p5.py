"""src/streaming/kafka_consumer_p5.py.
...
Terminal command to run this file from the root project folder:
    ...
    uv run python -m streaming.kafka_consumer_p5
...
OUTPUT_CSV: Final[Path] = OUTPUT_DIR / "consumed_sales_p5.csv"
...
def consume_messages(consumer: Any) -> int:
    ...
    while consumed_count < MAX_MESSAGES:
        ...
        processed["key_message_count"] = key_counts[key]
        ...
        LOG.info("Message count by Kafka key:")
        for key, count in key_counts.items():
            LOG.info(f"  key={key!r}: count={count}")
